// declare account ArgentX v0.4.0
// Launch with npx ts-node src/scripts/declareReadyClass.ts
// Coded with Starknet.js v6.23.1 & starknet-devnet.js v0.4.0
...
async function main() {
  ...
  if (respDecl.transaction_hash) {
    ...
    console.log("Account already declared.")
  };

const sha256Sierra = json.parse(fs.readFileSync("./src/scripts/argent_Sha256Cairo0.contract_class.json").toString("ascii"));
  // declare
  const respDecl2 = await account0.declareIfNot({ contract: sha256Sierra});
  let contractSHA256ClassHash: string;
  if (respDecl2.transaction_hash) {
    contractSHA256ClassHash = respDecl2.class_hash;
    // await myProvider.waitForTransaction(respDecl.transaction_hash);
    console.log("SHA256 Cairo0 contract declared. Class :", respDecl2.class_hash);
  } else {
    contractReadyClassHash = "0x04dacc042b398d6f385a87e7dd65d2bcb3270bb71c4b34857b3c658c7f52cf6dn"; 
    console.log("SHA256 already declared.")
  };
